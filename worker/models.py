...
class YT_API_Key:
    def __init__(self, key, created_at, daily_quota, last_used_timestamp, requests_left):
        self.key = key
    ...
    def daily_reset(self):
        last_used_timestamp_pacific = self.last_used_timestamp.astimezone(pytz.timezone('US/Pacific'))
        today_pacific = datetime.datetime.now().astimezone(pytz.timezone('US/Pacific'))
        today_pacific_start = datetime.datetime(today_pacific.year, today_pacific.month, today_pacific.day, 0, 0, 0, 0, pytz.timezone('US/Pacific'))
